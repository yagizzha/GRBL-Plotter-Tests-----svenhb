...
namespace GrblPlotter
{
    public partial class ControlCameraForm : Form
    {
        ...
        *******************************************************************/
        ...
        private void XDiffInp_TextChanged(object sender, EventArgs e)
        {
        ...
        private void ApplyTool_Click(object sender, EventArgs e)
        {
            ...
            if (XDiffInp.Text!="" && YDiffInp.Text!="")
            {
                Console.WriteLine("X"+XDiffInp.Text + "Y"+ YDiffInp.Text);
                Properties.Settings.Default.XToolDist = Convert.ToDouble(XDiffInp.Text);
                Properties.Settings.Default.YToolDist = Convert.ToDouble(YDiffInp.Text);
                XDiffInp.Text = "";
                YDiffInp.Text = "";
                Properties.Settings.Default.Save();
            }
            Console.WriteLine(Settings.XToolDist+" "+ Settings.YToolDist);
        }

        private void TestButton_Click(object sender, EventArgs e)
        {
            Settings Settings = new Settings();
            Console.WriteLine(Settings.XToolDist.ToString()+Settings.YToolDist.ToString());
            XyPoint ToolXY = new XyPoint(Settings.XToolDist, Settings.YToolDist);
            OnRaiseXYEvent(new XYEventArgs(0, 1, (XyPoint)Grbl.posWork, "G54"));//Swap to Tool
            OnRaiseXYEvent(new XYEventArgs(0, 1, (XyPoint)ToolXY, zeroCmd0));
            OnRaiseXYEvent(new XYEventArgs(0, 1, (XyPoint)Grbl.posWork, "G59"));//Swap to cam
        }
    }
}
